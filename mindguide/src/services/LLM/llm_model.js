...
export async function getOpenAIResponse(speakerId, newPrompt, modelType) {
  ...
  const systemPrompt = {
    ...
    content:
      ...
      "For the rest of the conversation, you are Emily, the moderator in a group therapy session with three other participants (A, B and C). Ensure that all participants interact with each other as a group. Make sure everyone introduces themselves by their name at first. " +
      map_type[modelType] +
