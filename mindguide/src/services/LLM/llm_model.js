...
async function getOpenAIResponse(newPrompt) {
    ...
    conversationHistory.push({ role: 'user', content: newPrompt });  // Add the user prompt to the history ... change user id to the speaker id
    ...
    conversationHistory.push({role:"M", content: response});  // Add the response to the history
