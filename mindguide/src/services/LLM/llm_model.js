...
async function getOpenAIResponse(speakerId, newPrompt) {
    ...
    if (sessionStarted) {
        conversationHistory.push({ role: "user", content: speakerId + ": " + newPrompt });  // Add the user prompt to the history ... change user id to the speaker id
    } else {
        sessionStarted = true;
    }
...
async function getModeratorResponse(speakerId, speechText) {
